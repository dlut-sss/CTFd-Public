...
function switchToCategory(chal) {
    ...
    $("#pages-board").find(pagebarid).addClass("active");
...
function renderSubmissionResponse(response) {
    ...
    const next_btn = $(
        `<div class='col-md-12 pb-3'><button class='challenge-submit btn btn-info w-100 '>` + (getCookieForLanguage("Scr1wCTFdLanguage") === "en" ? "Next Challenge" : "下一题") + `</button></div>`
    ).click(function () {
...
function loadChals() {
    return CTFd.api.get_challenge_list().then(function (response) {
        ...
        if (
            window.init.theme_settings &&
            window.init.theme_settings.page_bar_shown
        ) {
        ...
        for (let i = 0; i <= challenges.length - 1; i++) {
            ...
            if ($.inArray(chalinfo.category, pages) == -1) {
                ...
                const pagerow = $(
                    ...
                    'data-toggle="tab" role="tab" href="#{0}-row"'.format(pageid) +
                    '>' + page.slice(0, 15) + "</a>"
                );
                ...
                pagerow.on('shown.bs.tab', function () {
                    var element = $('#{0}-row'.format(pageid))[0];
                    var paddingTop = 28; // 你想要的上边距值
        ...
        //不重新遍历数据，用所有pages生成分类
        for (let i = 0; i <= pages.length - 1; i++) {
            const category = pages[i];
            const category_id = category.replace(/ /g, "-").hashCode();
            const category_row = $(
                "" +
                '<div id="{0}-row" class="pt-4" role="tabpanel"><div class="category-header col-md-12 mb-3"><h3>{1}</h3></div><div class="category-challenges col-md-12" id="{2}-base"><div class="challenges-row col-md-12" id="{3}-base-row"></div></div></div>'.format(category_id, category, category_id, category_id)
            );

            $challenges_board.append(category_row);
        }

        for (let i = 0; i <= challenges.length - 1; i++) {
            const chal_info = challenges[i];
            const chal_id = chal_info.name.replace(/ /g, "-").hashCode();
            const cat_id = chal_info.category.replace(/ /g, "-").hashCode();
            const chal_wrap = $(
                "<div id='{0}' class='col-md-3 d-inline-block challenge-button-container' ></div>".format(chal_id)
            );
            let chalbutton;

            if (solves.indexOf(chal_info.id) == -1) {
                chalbutton = $(
            ...
            const chalheader = $("<p>{0}</p>".format(chal_info.name));
            const chalscore = $("<span>{0}</span>".format(chal_info.value));
            for (let j = 0; j < chal_info.tags.length; j++) {
                const tag = "tag-" + chal_info.tags[j].value.replace(/ /g, "-");
                chal_wrap.addClass(tag);
            }

            chalbutton.append(chalheader);
            chalbutton.append(chalscore);
            chal_wrap.append(chalbutton);

            $("#" + cat_id + "-row")
                .find(".category-challenges")
                .find("#" + cat_id + "-base-row")
                .append(chal_wrap);
